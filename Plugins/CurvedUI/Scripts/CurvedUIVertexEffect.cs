...
#region CHECKS
        ...
        void CheckTextFontMaterial()
        {
            ...
            if (myText)
            {
                if (myText.cachedTextGenerator.verts.Count > 0 && (Vector2)myText.cachedTextGenerator.verts[0].uv0 != savedTextUV0)
                {
...
#region TESSELATION
        ...
        UIVertex TesselateQuad(UIVertex[] quad, float x, float y)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                uv0 += (Vector2) quad[i].uv0 * weights[i];
                uv1 += (Vector2) quad[i].uv1 * weights[i];
